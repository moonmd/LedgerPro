...
class AccountSerializer(serializers.ModelSerializer):
    ...
    def get_balance(self, obj): return obj.get_balance()
    ...
    def create(self, validated_data):
        ...
        try:
            for entry_data in journal_entries_data:
                ...
                if account.organization != organization: raise serializers.ValidationError(f'Account {account.name} invalid for org.')
            ...
            if total_debits != total_credits: raise serializers.ValidationError('Debits must equal Credits.')
        except serializers.ValidationError as e:
            transaction.delete()
            raise e
        AuditLog.objects.create(organization=organization, user=request.user, action='created_transaction', details={'transaction_id': str(transaction.id)})
        return transaction
...
class ReconciliationRuleSerializer(serializers.ModelSerializer):
    ...
    def validate_conditions(self, value):
        if not isinstance(value, list): raise serializers.ValidationError('Conditions must be a list.')
        for cond in value:
    ...
    def validate_actions(self, value):
        if not isinstance(value, list): raise serializers.ValidationError('Actions must be a list.')
        for act in value:
